...
namespace CustomCollection
{
    class Store<T> : IEnumerable<T>
    {
        ...
        public void Add(T item)
        {
            ...
            if (count > (int)Math.Ceiling((items.Length * 0.75)))
            {
                Console.WriteLine("Array at 75% capacity! Increasing array size by 50%!");
        ...
        public void Remove(T item)
        {
            ...
            for (int i=0; i < items.Length; i++)
            {
                if(EqualityComparer<T>.Default.Equals(items[i], item))
                {
                    ...
                    if (count < (int)Math.Ceiling((items.Length * 0.25)))
                    {
                        // if so create new array 1/2 size to transfer contents without item trying to remove.
                        Console.WriteLine("Array at 25% capacity! Decreasing array size by 50%!");
